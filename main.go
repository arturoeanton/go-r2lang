...
func main() {
	// Add error handling for R2Lang panics. Any panic reaching here — whether
	// wrapped with a call stack or a raw internal error (e.g. a type
	// conversion panic deep in the evaluator) — is reported as a clean
	// script error instead of a raw Go stack trace, so a bug in a user
	// script never leaks interpreter internals or crashes with an opaque
	// runtime.Error dump.
	defer func() {
		if r := recover(); r != nil {
