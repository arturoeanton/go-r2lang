...
func RegisterRand(env *Environment) {
	// randInit([seed])
	// Si se pasa un argumento numérico => se usa como semilla,
	// si no => se usa time.Now().
	env.Set("randInit", BuiltinFunction(func(args ...interface{}) interface{} {
		if len(args) == 0 {
			// seed con time
	...
	env.Set("randFloat", BuiltinFunction(func(args ...interface{}) interface{} {
		return rand.Float64()
	}))
