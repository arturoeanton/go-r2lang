// Sistema Contable LATAM - 100% FUNCIONAL
// POC para Siigo - Usando push() para todos los arrays

console.log("🌍 Sistema Contable LATAM - 100% Funcional")
console.log("📊 Libro Diario + DSL + API")
console.log("✅ Solución: usando push() para arrays en objetos")
console.log("")
...
// Función procesar transacción - USANDO PUSH
func procesarTransaccion(tipo, region, importe) {
    ...
    // IMPORTANTE: Usar push para agregar transacción
    ...
    // IMPORTANTE: Usar push para agregar movimientos
    if (tipo == "ventas") {
    ...
    // IMPORTANTE: Usar push para agregar asiento
...
func demoConsola() {
    ...
    while (i < std.len(asientosContables)) {
        let asiento = asientosContables[i]
        console.log("\nAsiento #" + (i + 1) + ": " + asiento.id + " - " + asiento.descripcion)
        console.log("Movimientos: " + std.len(asiento.movimientos))
...
// Parsear parámetros - decodificar URL
...
func handleHome(pathVars, method, body) {
    ...
    html = html + "input,select,textarea{width:100%;padding:8px;margin:5px 0;border:1px solid #ddd;border-radius:4px;box-sizing:border-box;}\n"
...
func handleProcesar(pathVars, method, body) {
    ...
    html = html + ".success{background:#d4edda;color:#155724;padding:15px;border-radius:8px;margin:15px 0;text-align:center;}\n"
    html = html + "</style>\n"
    html = html + "</head>\n<body>\n"
    
    html = html + "<div class=\"comp\">\n"
    html = html + "<h1>COMPROBANTE FISCAL</h1>\n"
    html = html + "<div class=\"success\">✅ Transacción procesada exitosamente</div>\n"
    ...
    if (totalDebe == totalHaber) {
        html = html + "<p style=\"color:green;text-align:center;margin-top:10px;\">✅ Asiento cuadrado</p>\n"
    }
...
func handleLibro(pathVars, method, body) {
    ...
    html = html + ".debe{color:#28a745;font-weight:bold;}\n"
    html = html + ".haber{color:#dc3545;font-weight:bold;}\n"
    html = html + ".empty{text-align:center;padding:40px;background:white;border-radius:8px;margin:20px 0;}\n"
    html = html + "</style>\n"
    html = html + "</head>\n<body>\n"
    ...
    if (std.len(asientosContables) == 0) {
        ...
        html = html + "<p>No hay asientos registrados aún.</p>\n"
        html = html + "<p><a href=\"/demo\">Ejecutar Demo</a> para generar asientos de ejemplo</p>\n"
        html = html + "</div>\n"
    } else {
        let i = 0
        while (i < std.len(asientosContables)) {
