// Sistema Contable LATAM - POC Final Funcionando
// Demo para Siigo - R2Lang + DSL
...
func procesarTx(tipo, region, importe) {
    console.log("Procesando: " + tipo + " " + region + " " + importe)
    ...
    // Crear transacción
    let txId = region + "-" + math.randomInt(9999)
    let tx = {
        id: txId,
        tipo: tipo,
        region: region,
    ...
    // Guardar transacción
    db.transacciones[std.len(db.transacciones)] = tx
    console.log("Transacción guardada. Total: " + std.len(db.transacciones))
    ...
    // Guardar asiento
    db.asientos[std.len(db.asientos)] = asiento
    console.log("Asiento guardado. Total: " + std.len(db.asientos))
...
dsl ReportesFinancieros {
    ...
    func ejecutar(cmd, tipo) {
        if (tipo == "balance") {
            ...
            let totalHaber = 0
            let i = 0
            while (i < std.len(db.asientos)) {
                let asiento = db.asientos[i]
                let j = 0
            ...
            return {
                ...
                totalAsientos: std.len(db.asientos),
        ...
        if (tipo == "ventas" || tipo == "compras") {
            ...
            let count = 0
            let i = 0
            while (i < std.len(db.transacciones)) {
                let tx = db.transacciones[i]
                if (tx.tipo == tipo) {
...
func handleHome(pathVars, method, body) {
    ...
    html = html + "<p style=\"text-align:center\">POC Completo - " + std.len(db.transacciones) + " transacciones - " + std.len(db.asientos) + " asientos</p>\n"
...
func handleProcesar(pathVars, method, body) {
    console.log("Handler procesar - body: " + body)
    ...
    console.log("Parámetros: tipo=" + tipo + " region=" + region + " importe=" + importe)
    
    // Procesar
    let tx = procesarTx(tipo, region, importe)
    
    // Verificar asiento
    let numAsientos = std.len(db.asientos)
    console.log("Número de asientos: " + numAsientos)
    
    if (numAsientos == 0) {
        return "<h1>Error: No se pudo crear el asiento</h1><p><a href=\"/\">Volver</a></p>"
    }
    
    let asiento = db.asientos[numAsientos - 1]
...
func handleLibro(pathVars, method, body) {
    ...
    html = html + "<p>Total asientos: " + std.len(db.asientos) + "</p>\n"
...
func handleDemo(pathVars, method, body) {
    ...
    html = html + "<p>Total: " + std.len(db.transacciones) + " transacciones</p>\n"
    html = html + "<p>Asientos: " + std.len(db.asientos) + " asientos</p>\n"
...
func handleAPITransacciones(pathVars, method, body) {
    return JSON({
        total: std.len(db.transacciones),
